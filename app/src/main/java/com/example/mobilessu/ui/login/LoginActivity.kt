...
class LoginActivity : AppCompatActivity() {
    ...
    protected fun doInBackground(vararg params: Void?): Boolean? {
        for (credential in DUMMY_CREDENTIALS) {
            ...
            if (pieces[0] == username.text.toString()) {
                // Account exists, return true if the password matches.
                return pieces[1] == password.text.toString()
            }
        }
    ...
    protected open fun onPostExecute(success: Boolean) {
       // mAuthTask = null
        //showProgress(false)
        if (success) {
            finish()
            val myIntent = Intent(this@LoginActivity, ActivityNews::class.java)
            this@LoginActivity.startActivity(myIntent)
        } else {
            password.setError("blah blah")
